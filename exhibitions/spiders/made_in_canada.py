...
class MadeInCanadaSpider(BaseSpider):
    ...
    def fetch_exhibitors(self, response: Response):
        ...
        next_page: Optional[str] = response.xpath(
    ...
    def parse_exhibitors(self, response: Response):
        ...
        exhibitor_name = response.xpath(
            '//*[@class="entry-content"]/p[contains(.//text(), "Name")]/text()'
        ).get()
        ...
        exhibitor_item.add_xpath(
            "category",
            '//*[@class="entry-content"]/p[contains(.//text(), "Products")]/text()',
        )
        exhibitor_item.add_xpath(
            "website",
            '//*[@class="entry-content"]/p[contains(.//text(), "Website")]/a/@href',
        )
        exhibitor_item.add_xpath(
            "description", '//*[@class="entry-content"]/p[not(b) and not(strong)][1]'
        )
