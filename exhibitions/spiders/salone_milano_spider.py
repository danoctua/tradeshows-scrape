...
class SaloneMilanoSpider(BaseSpider):
    ...
    EXHIBITOR_FETCH_START_LINK = (
        "https://www.salonemilano.it/it/brand?title=&sort_by=title&page=0"
    )
    ...
    def fetch_exhibitors(self, response: Response):
        exhibitors = response.xpath(
            "//*[contains(@class, 'list-flex-element-brands')]/a"
        )
        ...
        if next_page:
            yield response.follow(
                f'{response.url.split("?")[0]}{next_page}',
    ...
    def parse_exhibitors(self, response: Response):
        ...
        item_loader.add_xpath(
            "exhibitor_name", "//h1[contains(@class, 'heading-1')]/text()"
        )
        item_loader.add_xpath(
            "description",
