...
from exhibitions.item_loaders.furniture_manufacturer_loader import (
    FurnitureManufacturerItemLoader,
)
...
class FurnitureManufacturingSpider(BaseSpider):
    ...
    def fetch_exhibitors(self, response: Response):
        exhibitors = response.xpath(
            "//a[contains(@class, 'list__items__item__title__link')]"
        )
    ...
    def parse_exhibitors(self, response: Response):
        ...
        item.add_xpath(
            "exhibitor_name",
            '//h1[@class="m-exhibitor-entry__item__header__infos__title"]/text()',
        )
        ...
        item.add_xpath(
            "category",
            '//li[@class="m-exhibitor-entry__item__header__infos__categories__item"]/text()',
        )
        item.add_xpath(
            "description",
            '//div[@class="m-exhibitor-entry__item__body__description"]/p/text()',
        )
        item.add_xpath(
            "address",
            '//div[@class="m-exhibitor-entry__item__body__contacts__address"]/text()',
        )
        item.add_xpath(
            "website",
