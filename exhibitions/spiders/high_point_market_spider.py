...
class HighPointMarketSpider(BaseSpider):
    ...
    def fetch_exhibitors(self, response: Response):
        exhibitors = response.xpath(
            '//div[@class="exhibitor-row"]/a[contains(@id, "lnkExhibitor")]/@href'
        )
    ...
    def parse_exhibitors(self, response: Response):
        ...
        exhibitor_item.add_xpath(
            "exhibitor_name", '//*[@id="ctl00_ContentPlaceHolder1_lblName"]//text()'
        )
        exhibitor_item.add_xpath(
            "website", '//*[@id="ctl00_ContentPlaceHolder1_lnkWebsite"]//text()'
        )
        phone = response.xpath('//*[@id="exh-corpphone"]//text()').re_first(
            r"Corporate Phone:\s(.+)"
        )
        exhibitor_item.add_value("phone", phone)
        exhibitor_item.add_xpath("booth_number", '//*[@id="exh-showroom"]/span/text()')
        neighborhood = response.xpath('//*[@id="exh-neighborhood"]//text()').re_first(
            r"Neighborhood:\s(.+)"
        )
        exhibitor_item.add_value("hall_location", neighborhood)
        exhibitor_item.add_xpath("hall_location", '//*[@id="exh-shuttle"]//text()')
        exhibitor_item.add_xpath(
            "description", '//*[@id="ctl00_ContentPlaceHolder1_lblDescription"]//text()'
        )
